...
class Payment:
    """Rich domain aggregate representing a payment.

    Encapsulates lifecycle state transitions and guards them via domain rules.
    The application layer orchestrates persistence through a repository; the
    aggregate never touches infrastructure.
    """
    ...
    def attach_provider_reference(self, reference: str) -> None:
        if not reference:
            raise PaymentDomainError("Provider reference must be a non-empty string.")
        self._ensure_status(PaymentStatus.PENDING, action="Attach provider reference.")
        details = self.provider_details or PaymentProviderDetails()
    ...
    def mark_succeeded(self, provider_reference: str | None = None) -> None:
        self._ensure_status(PaymentStatus.PROCESSING, action="Mark payment as successful.")
    ...
    def mark_failed(self, reason: str) -> None:
        self._ensure_status(PaymentStatus.PROCESSING, action="Mark payment as failed.")
    ...
    def _ensure_status(self, *expected: PaymentStatus, action: str) -> None:
        ...
        raise PaymentDomainError(
            f"Cannot {action} from {self.status.value}. Expected one of: {expected_values}."
        )
