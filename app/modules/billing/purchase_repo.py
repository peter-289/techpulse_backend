...
class PurchaseRepository:
    ...
    async def find_by_payment(
        ...
        try:
            ...
            logger.exception(
                "Failed to fetch purchase by payment_id=%s",
                payment_id,
            )
            raise RepositoryUnavailableError(
                "Failed to retrieve purchase for payment."
            ) from exc
    ...
    async def save(self, purchase: Purchase) -> None:
         """
         Stage purchase for persistence. Caller must commit.
         """
    ...
    async def get(self, purchase_id: UUID) -> Purchase | None:
       try:
            stmt = (
                select(SoftwarePurchaseModel)
                .where(SoftwarePurchaseModel.id == str(purchase_id))
             )
    ...
    async def get_purchase(self, *, software_id: UUID, buyer_id: UUID) -> Purchase | None:
        """Get a purchase by buyer and software identifiers."""
        try:
            stmt = (
                select(SoftwarePurchaseModel)
                .where(
            ...
            logger.exception(
                "Failed to fetch purchase for software_id=%s buyer_id=%s",
                software_id,
                buyer_id,
            )
            raise RepositoryUnavailableError(
    ...
    async def list_owned(
        ...
        *,
        ...
        """This method lists all software purchases for a given buyer, returning a list of OwnedSoftwareCard objects and the total count of purchases."""
        try:
            buyer_key = str(buyer_id)
            where = SoftwarePurchaseModel.buyer_id == buyer_key
