...
from app.modules.shared.dependencies import require_role, get_current_user, CurrentUser, get_software_service
...
async def list_software(
    ...
    result: list[SoftwareRead] = []
    for item in items:
        has_access = await service.has_purchase(software_id=item.id, user_id=user_id)
        result.append(
            _software_item(
                item,
                viewer_user_id=user_id,
            ).model_copy(update={"viewer_has_access": item.price.amount_cents == 0 or item.owner_id == user_id or has_access})
        )
    return result
...
async def search(
    ...
    items = [
        _software_item(r.software, viewer_user_id=UUID(int=0))
        .model_copy(update={"viewer_has_access": r.software.price.amount_cents == 0 or r.software.owner_id == UUID(int=0)})
        for r in results
    ]
...
async def admin_summary(
    ...
    items = await service.list_visible(user_id=admin.user_id, limit=200)
