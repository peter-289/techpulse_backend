...
async def mark_verification_email_sent(user_id: int, sent_at: datetime | None = None) -> None:
...
async def mark_verification_email_failed(
    user_id: int,
    error_message: str,
...
async def process_unverified_users_once() -> None:
    ...
    try:
        uow = UnitOfWork(db)
        async with uow:
            users = await uow.user_repo.list_users_pending_verification_email_retry(
                now=now,
